export class PanchangCalculatorService {
  ...
  tipnaks: number[] = [
    2, 5, 6, 0, 1, 4, 3, 2, 4, 5, 5, 0, 2, 1, 3, 6, 1, 4, 4, 5, 0, 3, 3, 3, 5,
    0, 1,
  ];
  Lmoon: any;
  Lsun: any;
  ...
  calculate(d: Date): Panchang {
    ...
    var panchang: Panchang = {
      Ayanamsa: this.lon2dms(this.ayanamsa),
      Raasi: this.zn[z],
  ...
  moon(jd: any) {
    ...
    for (var i = 0; i < 93; i++) {
      // outrage at length
      var arg =
        this.corrMoon[i].mlcor * ml +
        this.corrMoon[i].mscor * ms +
        this.corrMoon[i].fcor * f +
        this.corrMoon[i].dcor * d;
      var sinarg = Math.sin(arg);
      if (this.corrMoon[i].mscor != 0) {
        sinarg *= i1corr;
        if (this.corrMoon[i].mscor == 2 || this.corrMoon[i].mscor == -2)
          sinarg *= i1corr;
      }
    ...
    for (var i = 0; i < 27; i++) {
      ...
      arg =
        this.corrMoon2[i].ml * ml +
        this.corrMoon2[i].ms * ms +
        this.corrMoon2[i].f * f +
        this.corrMoon2[i].d * d;
      sinarg = Math.sin(arg);
