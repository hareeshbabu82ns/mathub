...
import { ARITHMETIC_OPERATIONS_ALL, ARITHMETIC_OPERATIONS_DEFAULT } from "../../utils/gen_arithmetic_qa";
...
const SettingsPage = () => {
  ...
  const onSubmit = async (formData) => {
    ...
    navigate('new', { state: { qSettings: { ...formData, max: formData.totalQuestions }, replace: true } })
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={6}>
          <FormInputSlider
            name="maxDigits" label="Operand Digits"
            control={control} setValue={setValue}
            min={1} max={5} />
        </Grid>

        <Grid item xs={12} sm={6}>
          <FormInputCheckbox name="nonNegative" label="No -VEs"
            control={control} />
        </Grid>

        <Grid item xs={12} sm={6}>
          <FormInputCheckbox name="nonReminder" label="No Reminder Division"
            control={control} />
        </Grid>

        <Grid item xs={12} sm={6}>
          <FormInputSlider
            name="timeLimit" label="Time for Test (in Min)"
            control={control} setValue={setValue}
        ...
        <Grid item xs={12} sm={6}>
          <FormInputSlider
            name="timeLimitPerQuestion" label="Time Per Question (in Sec)"
            control={control} setValue={setValue}
            min={0} max={300} step={15} />
        </Grid>
        ...
        <Grid item xs={12} sm={6}>
          <FormInputMultiSelect
            name="includingOperations" label="Arithmetic Operations"
            control={control}
